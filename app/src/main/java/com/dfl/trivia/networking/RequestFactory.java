...
public class RequestFactory {
    ...
    public Flowable<TriviaCategoryList> getTriviaCategoryListRequest() {
        return NetworkModule.newInstance().getOpentdbApi().getTriviaCategoryList();
    }

    public Flowable<SessionTokenResponse> getSessionTokenRequest() {
        return NetworkModule.newInstance().getOpentdbApi().getSessionToken(Command.REQUEST.toString().toLowerCase());
    }

    public Flowable<ResetSessionTokenResponse> resetSessionTokenRequest(String sessionToken) {
        return NetworkModule.newInstance().getOpentdbApi().resetSessionToken(Command.RESET.toString().toLowerCase(), sessionToken);
    }

    public Flowable<QuestionsResponse> getQuestionsRequest(String token, int amount, int category, Difficulty difficulty, QuestionType questionType) {
        return NetworkModule.newInstance().getOpentdbApi().getQuestions(token, amount, category,
                difficulty.toString().toLowerCase(), questionType.toString().toLowerCase());
    }
}
